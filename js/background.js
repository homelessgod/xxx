...
var injectIntoTab = function(e) {
  ...
  var o = 0,
    n = t.length;
  for (; o < n; o++) {
    chrome.tabs.executeScript(e.id, { file: t[o] });
  }
};
...
chrome.extension.onMessage.addListener(function(e, t, o) {
  ...
  if (n) {
    ...
    if (n["scheduleDefaultLockOption"])
      f = "true" === n["scheduleDefaultLockOption"].usePMISchedule_locked;
  }
  if (e.type == "getZoomToken") {
    let { needPassword: e, voidable: t } = COMMON.checkNeedPasswordForPopup();
    o({
    ...
    if (e && e["meetingTrashUrl"]) {
      o({ meetingTrashUrl: COMMON.getUserBaseUrl() + e["meetingTrashUrl"] });
    }
  }
});
chrome.extension.onMessage.addListener(function(e, t, o) {
  ...
  if (e.type == "supportPAC") {
    ...
    chrome.tabs.create(
      { url: chrome.extension.getURL("permission.html"), selected: false },
      function(e) {
        chrome.tabs.update(e.id, { selected: true });
      }
    );
  } else if (e.type == "validateAltHost") {
    COMMON.validAlternateHost(
      e.althosts,
      function(e) {
        o(e);
      },
...
chrome.runtime.onConnect.addListener(function e(t) {
  ...
  connectPort = t;
  function o(e) {
    t.postMessage({ error: true, errorMsg: e });
  }
  function n(e) {
    COMMON.editMeeting(
      e,
  ...
  var i = 0;
  function a(e) {
    COMMON.scheduleMeeting(
      e,
      function(e) {
        COMMON.saveValue("zoom_config_fte", false);
        t.postMessage({ type: "schedule", meeting: e.result });
      },
      function(e) {
  ...
  function r(e, t) {
    if (!e || !e.length) {
      return zoomPort.postMessage({ type: "WCL.PermissionDeniedError" });
    }
  ...
  function s(e) {
    ...
    if (!isNaN(t)) {
      ...
      chrome.tabs.update(t, { active: true }, function(o) {
        let n = { type: "authenticationSigninResult", state: e };
        if (e === 2) {
  ...
  function c(e) {
    ...
    if (!isNaN(t)) {
      if (e === 1) {
        ...
        chrome.tabs.update(t, { active: true }, function(e) {
          if (!chrome.runtime.lastError) {
        ...
        let e = parseInt(COMMON.getValue("zoom_popup_tabpage_tabid"));
        chrome.tabs.update(e, { active: true }, function(e) {
          if (!chrome.runtime.lastError) {
  ...
  t.onMessage.addListener(function(e) {
    if (e.type == "WCL.getSourceID") {
      ...
      var f = COMMON.hasZoomRqToken();
      if (f) {
        COMMON.deleteMeeting(
          e.info,
          function(e) {
            t.postMessage({ type: "delSuccess", meeting: e.result });
          },
          function(e) {}
        );
      }
      ...
      COMMON.govLogin(
        ...
        function() {
          ...
          chrome.tabs.query({}, function(e) {
            if (e.length > 1) {
              chrome.tabs.remove(
                parseInt(COMMON.getValue("zoom_oauth_tabid")),
                function() {
                  if (chrome.runtime.lastError) {
                  }
                }
      ...
      COMMON.removeMeetingMp(
        e.info,
        function(e) {
          if (e.result && e.result != 0) {
            t.postMessage({ type: "deleteMeetingToast" });
          }
        },
        function() {}
      );
    } else if (e.type == "updateTimeAndTimezone") {
      COMMON.updateMeetingTimeAndTimezone(
        e.info,
        function() {},
        function() {}
      );
      ...
      COMMON.workemailSignin(e.info, function(e) {
        t.postMessage({ type: "workemailSigninResult", info: e });
      });
    } else if (e.type == "ssoSigninEvent") {
      COMMON.ssoSignin(e.info, function(e) {
        t.postMessage({ type: "ssoSigninResult", info: e });
      });
      ...
      t.postMessage({ type: "postAppConfig", info: APPCONFIG });
    }
  });
});
