...
(function(t) {
  ...
  function f() {
    return new Promise((t, e) => {
      chrome.runtime.sendMessage({ type: "loadAppConfig" }, e => {
  ...
  async function v() {
    ...
    t("body").on(
      "click",
      "[jsname=mg9Pef] [jsname=j7LFlb][aria-label=Delete]",
      function() {
        sendLogToBackend({
          msgType: 2,
    ...
    t("body").on(
      "click",
      ".Q3pIde [jsname=j7LFlb][aria-label=Delete]",
      function() {
        sendLogToBackend({
          msgType: 2,
          msg:
            'user clicks the Delete button in the menu "More actions" to delete event.'
        });
      }
    );
    t("body").on("keydown", function(e) {
      if (e.keyCode == 8 || e.keyCode == 46) {
        if (
          t("#yDmH0d [jsname=bN97Pc] div[jsname=SF0uGd][data-eventid]")
            .length ||
          t("#zoom-video-sec").length > 0 ||
          t("#b1w9Rc").length
        ) {
          return;
        }
    ...
    chrome.extension.sendMessage({ type: "getMeetingTrashUrl" }, function(t) {
  ...
  function g() {
    ...
    s = chrome.runtime.connect({ name: "Zoom" });
    s.onMessage.addListener(function(e) {
      ...
      if (e.type === "schedule" || e.type === "pmi") {
        ...
        if (t("#YPCqFe #zm-quick2adv-number").length <= 0) {
          t("#YPCqFe").append(
            '<div id="zm-quick2adv-number" style="display:none;">' +
              b(i.number) +
              "</div>"
          );
        } else {
        ...
        if (t("#YPCqFe #zm-usepmi").length <= 0) {
          t("#YPCqFe").append(
            '<div id="zm-usepmi" style="display:none;">' +
              b(i.usepmi) +
              "</div>"
          );
        } else {
        ...
        window.postMessage({ type: "WCL.PermissionDeniedError" }, "*");
      } else if (e.type == "WCL.userCancel") {
        window.postMessage({ type: "WCL.userCancel" }, "*");
  ...
  function b(t) {
    return DOMPurify.sanitize(t, { SAFE_FOR_JQUERY: true });
  }
  function _() {
    ...
    var i =
      t("body>input#zm_gov_url").val() || t("body>input#zm_web_domain").val();
    if (e && e.length > 32) {
      var n = t("body>input#zm_gov_url").val() || "";
      var o = /https?:\/\/\w*\.?zoomgov\.com\/?/gi.exec(n);
      if (n && o && o.length && o[0].indexOf("zoomgov.com") > 0) {
        chrome.extension.sendMessage({ type: "newPerm" }, function(t) {});
      } else {
        h({ type: "passOAuthToken", info: { token: e, url: i, term: false } });
      }
    }
    ...
    if (s && s.length > 8) {
      h({ type: "passSsoSession", info: { url: r } });
    }
    t(xhrObserver)
      ...
      .on("calendarIdRemoved", function(t) {
        if (m[t.event_baseid]) {
          h({
            ...
            info: { calendarId: t.calendarId, event_baseid: t.event_baseid }
          });
        ...
        e.forEach(function(e) {
          ...
          if (i && e.target == document.body && e.addedNodes[0].innerHTML) {
            ...
            if (n.indexOf('id="J9Hpafc') > 0) {
              ...
              var a = e.addedNodes[0].querySelector(
                "div[role=button][aria-describedby^=J9Hpafc]"
              );
              if (a && a !== null && o) {
                o.removeChild(a);
              }
          ...
          if (i && i.search('role="dialog"') !== -1) {
            if (
              ...
              t("div[jsname=troJl] div[jsname=V68bde]").delegate(
                "div[role=option]",
                "click",
                function(e) {
                  t(this)
              ...
              t("div[jsname=troJl] div[jsname=V68bde]").delegate(
                ...
                function(e) {
                  if (e.keyCode == 13) {
                    var i = t(this)
                      .find("div[role=option][tabindex=0]")
                      .attr("data-value")
                      .replace("suggestion:", "");
                    if (i)
                      t(this)
              ...
              t("div[jsname=c6xFrd]").delegate(
                ...
                function(e) {
                  ...
                  if (i) {
                    t("div[jsname=L2QMlb] div[jsname=yxQRke]").attr(
                      "timezoneid",
                      i
                    );
                  } else {
              ...
              var s = Q();
              if (
                i.search('jsname="c6xFrd"') !== -1 &&
                s &&
                s.url &&
                i.search('jsname="VGHUeb"') == -1 &&
                i.search('jsname="AOxYWc"') == -1
              ) {
                var r = e.addedNodes[0];
                var d = r.querySelector("span[jsname=bN97Pc] >div");
                if (
                  d &&
                  d.innerHTML.search("zm-remove-line") == -1 &&
                  r.querySelector("div[jsname=d8Aqse]  textarea[jsname=YPqjbf]")
                ) {
                  ...
                  g.appendChild(
                    document.createTextNode(b.text() + " " + _.text())
                  );
                  f.appendChild(v);
                  f.appendChild(g);
                ...
                t("div[jsaction][jsname=c6xFrd]").delegate(
                  "div[role=button]",
                  "click",
                  function(e) {
                    ...
                    if (i === "di8rgd" || i === "deA65") {
                      ...
                      if (n && n.number) {
                        ...
                        h({ type: "delete", info: { number: n.number } });
                      }
                    }
                  }
                );
              ...
              if (i.search('jsname="bN97Pc"') !== -1) {
                var y = t(
                  "#yDmH0d [jsname=bN97Pc] div[jsname=SF0uGd][data-eventid]"
                );
                if (y.length > 0) {
                  var k = [];
                  k.push(
                  ...
                  if (y.find(".quick-schedule").length <= 0) {
                    y.find("div[jsname=x8hlje]")
                      .removeClass("zZhnYe e3Duub")
                      .addClass("nYqxP I2LGc");
                    y.find("div[jsname=x8hlje]")
                      .parent()
                      .append(k.join(""));
                  ...
                  t("div.quick-schedule content>a").delegate(
                    "",
                    "click",
                    function() {
                      var e =
                        t("div[jsname=oAi9S] #xDetDlgVideo[data-details]").attr(
                          "data-details"
                        ) || "";
                      var i = X(e);
                      let n = l.getDescNode();
                      ...
                      var o = this;
                      var a = t(
                        "div[jsname=vHVaxd] input[role=combobox][jsname=YPqjbf]"
                      );
                      var s = t(
                        "div[jsname=wU3vpb] div[jsname=ovr9Gb] div[role=button][jsname=ix0Hvc]"
                      );
                      if (
                        (s.is(":visible") || a.is(":visible")) &&
                        i &&
                        i.url
                      ) {
                        o.lock = true;
                        var r =
                          t(
                            "div[jsname=nw2lBd] a[href*='/www.google.com/url?']"
                          ).attr("href") || "";
                        var d =
                          "<br>" +
                          "─".repeat(10) +
                          ...
                          '<a href="' +
                          r +
                          '" target="_blank">Joining Instructions</a>' +
                          "<br><br>" +
                          "─".repeat(10);
                        var c = t.trim(n.html());
                        if (c && "" != n.text()) {
                        ...
                        t("div[jsname=ovr9Gb] div[jsname=vRNCKf]").addClass(
                          "sMVRZe"
                        );
                        t(
                          "div[jsname=ovr9Gb] div[jsname=vRNCKf] div[role=button][jsname=ix0Hvc]"
                        ).trigger("click");
                        ...
                        chrome.extension.sendMessage(
                          ...
                          function(e) {
                            ...
                            if (e && e.status) {
                              if (
                                y.find("div[jsname=GYcwYe] input").val() == ""
                              ) {
                                y.find("div[jsname=GYcwYe] input")
                                  .val(e.result.topic)
                                  .focus();
                                y.find(
                                  "div[jsname=GYcwYe] input"
                                )[0].dispatchEvent(
                              ...
                              if (e.result.addJoinUrl) {
                                ...
                                if (r.is(":visible") || s.is(":visible")) {
                                  ...
                                  if (i.length <= 0) {
                                    t("#yDmH0d").append(
                                      '<div id="zm-quick-location" style="display:none;">' +
                                        (d || "") +
                                        "</div>"
                                    );
                                  } else {
                              ...
                              if (t("#zm-quick2adv-number").length <= 0) {
                                ...
                                t("#YPCqFe #zm-quick2adv-number").text(
                                  e.result.number || ""
                                );
                              }
                              var c = l.getDescNode();
                              if (c.length > 0 && c.is(":visible")) {
                                ...
                                var p =
                                  encodeURIComponent(e.result.email) || "";
                                p =
                                  "\n" +
                                ...
                                if (n.length <= 0) {
                                  t("#yDmH0d").append(
                                    '<div id="zm-quick-desc" style="display:none;">' +
                                      p +
                                      "</div>"
                                  );
                                } else {
          ...
          if (
            i &&
            i.search('id="xDetDlgVideo"') !== -1 &&
            "EVENT_EDIT" == document.querySelector("body").dataset.viewfamily
          ) {
            var s = W();
            if (s && s.number) {
              var z =
                t("#xRTCIn #xDetDlgVideo[data-details]").attr("data-details") ||
                "";
              var w = X(z);
          ...
          if (x) {
            if (x.search('role="dialog"') !== -1) {
              if (
                ...
                if (
                  x.search('jsname="bN97Pc"') !== -1 &&
                  x.search('class="JtukPc"') !== -1
                ) {
                  setTimeout(function() {
              ...
              x.search('role="button"') !== -1 &&
              x.search("data-eventid=") !== -1 &&
              x.search('data-opens-details="true"') !== -1 &&
              x.match(/jsaction=/g).length > 1 &&
              x.search('role="main"') === -1 &&
              x.search("jsaction=") < x.search("ynRLnc")
            ) {
              let e = new RegExp('(?<=data-eventid=")[a-zA-Z0-9/+]+(?=")').exec(
                x
              );
              if (t.isArray(e)) {
                e = e[0];
              ...
              "EVENT_EDIT" ==
                document.querySelector("body").dataset.viewfamily &&
              x.search('jsname="aZ2wEe"') !== -1
            ) {
              var s = W();
              if (s && s.number) {
                var z =
                  t("#xRTCIn #xDetDlgVideo[data-details]").attr(
                    "data-details"
                  ) || "";
                var w = X(z);
                if (w && w.number && w.number !== s.number) {
                  ...
                  sendLogToBackend({
                    ...
                    msg: {
                      msg:
                        "automatically delete add-on meeting that conflict with extension meeting",
                  ...
                  h({ type: "deleteonly", info: { number: w.number } });
                }
              }
            } else if (
              "EVENT_EDIT" ==
                document.querySelector("body").dataset.viewfamily &&
              x.search('id="xDetDlgVideo"') !== -1
            ) {
              var s = W();
              if (s && s.number) {
                var z = "";
                var C = e.removedNodes[0].querySelector(
                  "#xDetDlgVideo[data-details]"
                );
                if (C) {
                ...
                if (w && w.number && w.number == s.number) {
                  ...
                  h({ type: "delete", info: { number: w.number } });
                }
              }
            }
          }
            ...
            if (n && n.length > 0 && i.is(":visible")) {
              ...
              if (i.length > 0) {
                ...
                if (a && a.url) {
                  ...
                  s.push(
                    '\t\t<div class="zoom-pop-link"><a role="button" class="btn-small btn-meeting" target="_blank">Join Zoom Meeting</a></div>'
                  );
                  s.push("\t</div>");
                  s.push("</div>");
                  t("[jsname=bN97Pc] #xDtlDlgCt .kMp0We:eq(0)").after(
                    s.join("")
                  );
                  t(".zoom-pop-link>a")[0].setAttribute("href", b(a.url));
                  t("#xDetDlg div[role=button][jsname=VkLyEc]").delegate(
                    "",
                    "click",
                    function(t) {
                      if (!o) {
                        sendLogToBackend({
                          ...
                          msg: {
                            msg:
                              "in the event preview popover, the user clicks the delete button",
                            meetingNumber: a.number,
                            nodeId: this.id
                          },
                          operaType: 1
                        });
                        h({ type: "delete", info: { number: a.number } });
                      }
                    }
                  );
      ...
      if (t("#maincell #coverinner")) {
        ...
        var g = new y(function(e) {
          ...
          if (t(".ep-etz-dialog").is(":visible")) {
            ...
            t(".ep-etz-dialog .ep-etz-dialog-buttons").delegate(
              ...
              function(e) {
                ...
                if (n) {
                  t("span[id$=\\-edr-st-txt], span[id$=\\-edr-both-txt]").attr(
                    "timezoneid",
                    n
                  );
                }
            ...
            t(".ep-etz-dialog .ep-etz-dialog-buttons").delegate(
              ...
              function(e) {
                t(
                  "span[id$=\\-edr-st-txt], span[id$=\\-edr-both-txt]"
                ).removeAttr("timezoneid");
              }
            );
            t(".ep-etz-dialog .tz-pck-timezone").unbind();
            t(".ep-etz-dialog .tz-pck-timezone").delegate(
              "",
              "change",
              function(e) {
                var i = t(".ep-etz-dialog .ep-etz-box-start .tz-pck-timezone");
          ...
          if (t(".ep-etz-remove-dialog").is(":visible")) {
            ...
            t(".ep-etz-remove-dialog .ep-etz-remove-dialog-buttons").delegate(
              "> button[name=ok]",
              "click",
              function(e) {
                t(
        ...
        if (v) {
          g.observe(v, { attributes: false, childList: true, subtree: true });
        }
      }
    }
    l = uiOperation;
  ...
  function z(e) {
    ...
    i.zoom_config_video_participants = n
      .find("[name=option_video_participants]:checked")
      .val();
    i.zoom_config_audio = n.find("[name=option_audio]:checked").val();
    i.zoom_config_jbh = n.find("#setting_option_jbh").prop("checked");
    i.password = n.find("#setting_option_needpassword").is(":visible")
      ? n.find("[name=meeting_pwd]").val()
      : "";
    ...
    i.zoom_config_pmi = !!(
      n.find("[name=meeting_type]:checked").val() === "pmi"
    );
    i.zoom_config_alternativehost = t("#alternative_host").is(":visible")
      ? t("#alternative_host").val()
      : "";
    i.scheduleTime = I();
    i.trackfields = getTrackfieldValue(t("input[name=trackfield]"));
    i.zoom_config_watermark = !!n
      .find("#setting_option_watermark")
      .prop("checked");
    i.zoom_option_require_reg = !!n
      .find("#setting_option_requirereg")
      .prop("checked");
    let o = n.find("#JBH_prior_start");
    if (!i.zoom_config_pmi && i.zoom_config_jbh && o.is(":visible")) {
      i.jbhTimeMinutes = o.val();
    }
    i.enableAuthentication =
      n.find("#setting_option_authentication").is(":visible") &&
      n.find("#setting_option_authentication").prop("checked");
    if (i.enableAuthentication) {
      i.updateAuthOptionItem = ie();
    }
    if (
      t("#zoom-setting #meeting_type_pmi").prop("checked") &&
      t("#zoom-setting #setting_option_needpassword").prop("checked")
    ) {
      i.zoom_config_pmi_saved_pwd = n
        .find("[name=meeting_pwd]")
        .val()
        .trim();
    }
    i.zoom_config_audiomark = !!n
      .find("#setting_option_audiomark")
      .prop("checked");
    i.record_type = e;
  ...
  function w(e, i) {
      ...
      t.each(o, function(i, o) {
        ...
        if (o.id == n) {
          ...
          if ("000-000-000" !== o.roomid) {
            ...
            t("[name=meeting_type][value=onetime]").prop({ checked: true });
          }
        }
      });
    } else {
    ...
    if (i && i.schedulePmiLock) {
      t("[name=meeting_type][value=pmi]").prop({ checked: true });
      t("[name=meeting_type]").prop({ disabled: true });
      t(".pmi")
        .append(r)
        .addClass("haslock");
    }
    ...
    if (!l) {
      e.find(".audio #option_audio_3rd")
        .parent()
        .hide();
      if ("other" == s) s = "both";
      e.find(".audio #option_audio_both")
        .next()
        .text("Both");
    }
    ...
    e.find("[name=option_video_participants][value=" + h + "]").prop(
      "checked",
      true
    );
    ...
    if (!_) {
      t("#setting_option_public_cal")
        .parents(".checkbox-inline")
        .hide();
    } else {
      ...
      if (i.zoom_local_record && i.zoom_cloud_record) {
        if (t("#setting_recording_type").prop("checked")) {
          ...
          if (2 == k) {
            ...
            t("[name=option_rec_type][value=1]").prop({ checked: true });
          }
        }
      } else if (i.zoom_local_record) {
        t("#setting_recording_type")
          .next()
          .text(APPCONFIG.tips[4]);
        t(".recording-radio").remove();
      } else {
        t("#setting_recording_type")
          .next()
          .text(APPCONFIG.tips[5]);
        t(".recording-radio").remove();
      }
    }
    if (i.lockOpts) {
      ...
      if (z.hostVideo_locked) {
        ...
        t(".video .video-controls-label:first")
          .append(r)
          .addClass("haslock");
      }
      if (z.pVideo_locked) {
        t("[name=option_video_participants]").prop("disabled", true);
        t(".video .video-controls-label:last")
          .append(r)
          .addClass("haslock");
      }
      if (z.audio_locked) {
        t("[name=option_audio]").prop("disabled", true);
        t(".audio .controls[role=group]")
          .append(r)
          .addClass("haslock");
      }
      if (z.jbh_locked) {
    ...
    if (i && i.option_enable_reg === true) {
      if (i.schedulePmiLock) {
        e.find("#setting_option_requirereg").prop({ disabled: true });
      } else if (i.zoom_option_require_reg === true) {
        e.find("#setting_option_requirereg")
          .prop({ checked: true })
  ...
  function x(e, i) {
    ...
    o.push(
      '\t\t\t\t\t\t\t\t<label class="left-label control-labe" for="trackitem' +
        b(e.id) +
        '" >' +
        b(e.trackField)
    );
    ...
    o.push(
      '\t\t\t\t\t\t\t\t\t\t<input type="text" id="' +
        ...
        "trackitem" +
        b(e.id) +
        '" name="trackfield" aria-required="' +
        b(e.required) +
        '" require="' +
        b(e.required) +
        '" maxlength="50" autocomplete="off" value="' +
        b(e.mtValue) +
        '" class="form-control" style="height:25px;"></input>'
    );
  ...
  function j(e) {
    h({ type: "sync", info: {} });
    chrome.extension.sendMessage({ type: "getZoomToken" }, function(i) {
      ...
      if (i.token) {
        if (i.fte || i.showSetting) {
          t.modal(
            ...
            t.extend(
              {},
              {
                ...
                onShow: function(n) {
                  p.$authentication = t(
                    "#zoom-setting .control-row.authentication"
                  );
                  h({ type: "loadZoomOptions" });
                  let o = t("#zoom-setting .password-type");
                  t("#zoom-setting .submit").bind("click", function() {
                    t("#zoom-setting #alternative_host").trigger("blur");
                    if (
                      t("#zoom-setting #setting_option_needpassword").prop(
                        "checked"
                      ) &&
                      t("#meeting_pwd").is(":visible")
                    ) {
                      if (
                        "" ==
                        t("#meeting_pwd")
                    ...
                    if (
                      ...
                      if ("" != t("#zoom-setting #alternative_host").val()) {
                        t("#zoom-setting .modal-footer .submit").addClass(
                          "inprogress"
                        );
                        ...
                        chrome.extension.sendMessage(
                          ...
                          function(i) {
                            ...
                            t(
                              "#zoom-setting .modal-footer .submit"
                            ...
                            if (
                              ...
                              h({ type: "saveOptionsAndSchedule", info: z(n) });
                              t.modal.close();
                              lt(e);
                            }
                          }
                  ...
                  t("#zoom-setting #setting_option_needpassword").bind(
                    "change",
                    function() {
                      var e =
                        t("#zoom-setting #setting_option_needpassword").prop(
                  ...
                  t("#zoom-setting #alternative_host").bind("blur", function(
                    e
                  ) {
                    var i = t(this);
                    i.val(i.val().trim());
                  });
                  t("#zoom-setting #alternative_host").bind("keyup", function(
                    e
                  ) {
                    var i = t(this);
                    var n = i.val();
                  ...
                  t("#zoom-setting #setting_option_requirereg").bind(
                    "change",
                    function() {
                      var e =
                        t("#zoom-setting #setting_option_requirereg").prop(
                          "checked"
                        ) || false;
                      if (
                        e &&
                        ...
                        t("#zoom-setting #meeting_type_normal").prop(
                          "checked",
                          true
                        );
                        t("#zoom-setting #meeting_type_normal").trigger(
                          "change"
                        );
                      }
                    }
                  ...
                  t("#meeting_pwd")
                    .next()
                    .hide();
                  t("#zoom-setting [name=meeting_type]").bind(
                    "change",
                    function() {
                      let e =
                        t("#zoom-setting #meeting_type_pmi").prop("checked") ||
                        false;
                      if (e) {
                        t("#zoom-setting #setting_option_requirereg")
                          .prop("checked", false)
                          .attr("checked", null);
                  ...
                  t("#zoom-setting #setting_option_jbh").bind(
                    "change",
                    function() {
                      if (
                        d &&
                        t("[name=meeting_type]:checked").val() === "pmi"
                      ) {
                        Et(true);
                      }
                      if (t(this).prop("checked")) {
                        t("#zoom-setting #JBH_prior_start")
                          .prop("disabled", false)
                          .css("opacity", 1);
                      } else {
                        t("#zoom-setting #JBH_prior_start")
                ...
                onClose: function(e) {
                  ...
                  t("#zoom-setting #setting_option_needpassword").unbind(
                    "change"
                  );
                  ...
                  t("#zoom-setting #setting_option_requirereg").unbind(
                    "change"
                  );
                  ...
                  e.container.fadeOut("fast", function() {
                    e.overlay.fadeOut("fast", function() {
                      t.modal.close();
                      if (
                        t("#zoom_schedule_button") &&
                        t("#zoom_schedule_button").is(":visible")
                      ) {
          ...
          if (i.requireTracking || i.inputManualPwd) {
            t.modal(
              t(C(i.requireTracking, i.inputManualPwd, i.pwdVoidable)),
              t.extend(
                {},
                {
                  ...
                  onShow: function(n) {
                    ...
                    t("#zm-track-dialog-container #zoom-setting .submit").bind(
                      "click",
                      function() {
                        if (validatePwdInput() && validateTrackCodes()) {
                          ...
                          let l = {
                            scheduleTime: I(),
                            trackfields: getTrackfieldValue(
                              t("input[name=trackfield]")
                            ),
                            timezone: V(),
                    ...
                    if (l && l.length > 0) {
                      t(
                        "#zm-track-dialog-container .trackingcodes > .trackitems"
                      ).empty();
                  ...
                  onClose: function(e) {
                    t(
                      "#zm-track-dialog-container #zoom-setting .submit"
                    ).unbind("click");
                    t("#zoom-setting #setting_option_needpassword").unbind(
                      "change"
                    );
            ...
            h({ type: "schedule", info: { scheduleTime: I(), timezone: V() } });
            lt(e);
          }
        }
      } else {
    ...
    chrome.extension.sendMessage({ type: "getZoomToken" }, function(i) {
      ...
      if (n) {
        if (i.requireTracking) {
          t.modal(
            ...
            t.extend(
              {},
              {
                      ...
                      if (validateTrackCodes()) {
                        h({
                          ...
                          info: {
                            trackfields: getTrackfieldValue(
                              t(
                                "#zm-track-dialog-container input[name=trackfield]"
                              )
                            )
                          }
                        });
                ...
                onClose: function(e) {
                  t("#zm-track-dialog-container #zoom-setting .submit").unbind(
                    "click"
                  );
                  e.container.fadeOut("fast", function() {
                    e.overlay.fadeOut("fast", function() {
          ...
          h({ type: "schedulepac", info: { trackfields: "" } });
          lt(e);
        }
      } else {
        ft();
  ...
  function L(t) {
    ...
    if (O(n)) {
      ...
      sendLogToBackend({
        ...
        msg: {
          msg:
            "Google calendar time format changed, unable to complete conversion!, method name: calendarTimeConver",
          str: t,
          res: n
        },
        operaType: 4
  ...
  function H() {
    ...
    if (!e.length) {
      ...
      if (i.length) {
        e = t(
          '<a id="zoom_schedule_meeting_url" role="button" class="btn-small btn-meeting hideme" target="_blank"></a>'
        ).appendTo(i);
      }
    }
    return e;
  ...
  function X(t) {
    ...
    var e;
    var i = new RegExp(
      `(?<!=)https?:\\/\\/[A-Za-z0-9_-]*\\.?zoom(${APPCONFIG.runEnv[1]})?\\.(us|com.cn|com)\\/[jsw]\\/(\\d+)(\\?\\w+=[^ <|\\n]*)?`,
      "ig"
    ).exec(t);
    if (i && i.length) {
      var n = i[0].replace("</a>", "");
      var o = i[3];
      e = { number: o, url: n.replace("<br>", ""), type: "meeting" };
    } else {
      var a = new RegExp(
        `https?:\\/\\/[A-Za-z0-9_-]*\\.?zoom(${APPCONFIG.runEnv[1]})?\\.(us|com.cn|com)\\/pac\\/join\\/(\\d+)(\\?\\w+=[^ |\\n]*)?`,
        "ig"
      ).exec(t);
      if (a && a.length) {
        e = {
    ...
    if (!e) {
      ...
      if (s && s.length) {
        e = { number: s[1], url: s[0].replace("<br>", ""), type: "meeting" };
      } else {
    ...
    if (!i.data("click-event-registered")) {
      ...
      i.unbind("click").click(function() {
        ...
        if (i) {
          ...
          if (r.length) {
            t.extend(s, { topic: r.val() });
          }
          var d = A();
          if (d) {
            var c = $();
            t.extend(s, { startTime: toISOString(d), duration: c });
          } else {
            t.extend(s, { startTime: "", duration: 0 });
          }
          var m = l.isRepeat();
          t.extend(s, {
            isRepeat: m,
  ...
  function nt(e) {
    ...
    if (t("#zoom_schedule_button_wrapper > #zoom_schedule_button").length > 0) {
    } else {
    ...
    chrome.extension.sendMessage({ type: "supportPAC" }, function(e) {
      if (e.hasPac) {
        var i = t("<button>", {
  ...
  function at() {
    ...
    e.push('\t<div class="">');
    e.push(
      '\t\t<span class="" aria-hidden="true">Your Zoom Basic plan has a 40-minute time limit on meetings with 3 or more participants. <a href="https://zoom.us/buy?plan=pro&from=pro&zcid=2166&utm_source=Product&utm_medium=email&utm_content=FreeToPro&utm_campaign=ChromeExtension" class="zoom-upgrade" target="_blank"> Upgrade now </a> to enjoy unlimited group meetings.</span>'
    );
    e.push("\t</div>");
    e.push("</div>");
  ...
  function st(e) {
    ...
    if (i.children()[0]) {
      var n = t(
        '<div id="zoom-video-sec" class="' +
          b(i.children()[0].className) +
          ' zoom-video-sec"></div>'
      );
      ...
      if (
        ...
        l.setDescNodeValue(
          document.querySelector("#zm-quick2adv-desc").innerHTML
        );
      }
      var o = location.pathname.split("eventedit/");
      if (t("#zm-hidden-eid").length <= 0) {
        t("#YPCqFe").append(
          '<div id="zm-hidden-eid" style="display:none;"></div>'
        );
      }
      t("#YPCqFe #zm-quick2adv-number").empty();
      if (
      ...
      if (check40MinDuration()) {
        chrome.extension.sendMessage({ type: "checkUserType" }, function(t) {
          if (t.freeUser) {
            at();
          }
        });
      ...
      if (a) {
        ...
        if (
          ...
          c.prepend(
            t('<div id="zoom_schedule_button_wrapper"></div>').prepend(m)
          );
        }
        ...
        m.unbind("click").click(function() {
          ...
          if (i && i.url) {
            ...
            var o =
              t("#xRTCIn a[href*='/script.google.com/']").attr("href") || "";
            l.setDescNodeValue(
              i.url +
                "<br><br>" +
                '<a href="' +
        ...
        chrome.extension.sendMessage({ type: "supportPAC" }, function(e) {
          if (e.hasPac) {
            ...
            if (
              t("#zoom_schedule_button_wrapper > #zoom_schedulepac_button")
                .length > 0
            ) {
            } else {
              u.append(i);
    ...
    if (i) {
      if (t("#zoom_schedule_meeting_url + i.icon-trash").length <= 0) {
        var n =
          t("#xRTCIn #xDetDlgVideo[data-details]").attr("data-details") || "";
        var o = X(n);
        if (o && o.number && o.number == i.number) {
        } else {
          e.parent().append(
            '<i class="icon-trash" title="Remove Zoom Meeting" aria-label="Remove Zoom Meeting"></i>'
          );
        }
      }
      t("#zoom_schedule_meeting_url + i.icon-trash").unbind();
      t("#zoom_schedule_meeting_url + i.icon-trash").delegate(
        "",
        "click",
        function(e) {
          if (i && i.number) {
            ...
            h({ type: "icon_delete", info: { number: o, event_baseid: a } });
            t("#YPCqFe #zm-usepmi").empty();
            t("#YPCqFe #zm-quick2adv-number").empty();
          }
        }
  ...
  function pt() {
    var e = [];
    t(
      "div[jsname=wU3vpb] div[jsname=ovr9Gb] div[jsname=ix0Hvc] div.gco0Sd"
    ).each(function(i, n) {
    ...
    n.on("keyup", "input", function(i) {
      ...
      s = setTimeout(function() {
        if (
          ...
          t("#zoom-input-password").val().length >= 6
        ) {
          e.find(".btn-workemail-signin")
    ...
    n.on("click", ".btn-workemail-signin", function(e) {
      ...
      if (isEmail(s) && l.length >= 6) {
        ...
        h({
          ...
          info: { email: s, password: l, type: 100 }
        });
        n.lock = true;
      }
    });
    let l,
      r = o.find(".continue > button"),
      d = o.find(".sso-msg");
      ...
      if (
        ...
        h({
          ...
          info: {
            ...
            domain: a.is(":visible")
              ? a.val() + `.zoom${APPCONFIG.runEnv[1]}.us`
              : ""
          }
        });
      }
  ...
  function Et(e, i) {
    ...
    t("[name=option_password_type][value=" + a + "]").prop({ checked: true });
    if (o) {
  ...
  function Mt(e, i) {
    let n =
      t("#zoom-setting #setting_option_needpassword").prop("checked") || false;
    let o = t("#zoom-setting .password-type");
    if (n) {
      o.addClass("sel");
    } else {
  ...
  function $t() {
    ...
    if (e.support) {
      ...
      let { htmlText: t, selVal: i } = Jt(p.renderData);
      p.$authentication.html(t);
      Rt();
      Vt(i, e);
    } else {
      p.$authentication.html("").hide();
      t("#zoom-setting .control-row.watermark")
        .html("")
        .hide();
      t("#zoom-setting .control-row.audiomark")
        .html("")
        .hide();
    }
  }
  ...
  function Rt() {
    ...
    p.$authentication.on("click", ".domain-info-field .edit-button", function(
      t
    ) {
      t.stopPropagation();
      Qt();
  ...
  function Bt() {
    ...
    if (e.is(":visible")) {
      ...
      if (
        p.$authentication.position().top * 2 >
        t("#zoom-setting .modal-body").height()
      ) {
  ...
  function Zt(t, e) {
    ...
    if (i.type == "1") {
      ...
      let a = `\n\t\t\t<span class="domain-info-text">${b(
        e
      )}</span><span class="edit-button">${o}</span>\n\t\t\t`;
      p.$authentication
        .find(".domain-info-field")
        .html(a)
        .addClass("eyeable");
    } else {
      p.$authentication
  ...
  function ne() {
    if (p.$zmEditDomainModule) {
      ...
      if (e.is(":visible") && !e.prop("disabled")) {
        toolFun.showHasErrorMsg(
          t("#zoom-setting #auth_domain_info"),
          APPCONFIG.errors[17]
        );
        return false;
      }
