...
$(function() {
  ...
  function g(e, t, o, i) {
    ...
    o.html(e);
    var a =
      (parseInt($(document.body).outerWidth(true)) - n.outerWidth(true)) / 2;
    if (o === n) {
      o.css("left", a);
    }
    if (h) {
  ...
  function C() {
    ...
    if (
      t.is(":visible") &&
      $("#setting_option_authentication").prop("checked")
    ) {
      if (!q(t.val())) {
  ...
  function N() {
    ...
    e.onreadystatechange = function() {
      if (e.readyState == 4) {
        try {
          ...
          $(".video .control-row .video-label")
            .first()
            .text(x("setting_video_host", t));
          $(".video .control-row .video-label")
            .last()
            .text(x("setting_video_participant", t));
          $(".video #option_video_host_on")
            .next()
            .text(x("setting_video_on", t));
          $(".video #option_video_participant_on")
            .next()
            .text(x("setting_video_on", t));
          $(".video #option_video_host_off")
            .next()
            .text(x("setting_video_off", t));
          $(".video #option_video_participant_off")
            .next()
            .text(x("setting_video_off", t));
          $(".audio > span").text(x("setting_audio", t));
          $(".audio #option_audio_telephony")
            .next()
            .text(x("setting_audio_tel", t));
          $(".audio #option_audio_voip")
            .next()
            .text(x("setting_audio_voip", t));
          $(".options > span").text(x("setting_opts", t));
          $(".options #setting_option_needpassword")
            .next()
            .text(x("setting_opts_requirepwd", t));
          $(".options #setting_option_jbh")
            .next()
            .text(x("setting_opts_jbh", t));
    ...
    e.open(
      ...
      chrome.extension.getURL(
        "/_locales/" + COMMON.getValue("zoom_locale") + "/messages.json"
      ),
      true
    );
    e.send();
  ...
  function y() {
    if (
      e &&
      !!e["option_pmi_jbhpwd"] &&
      $("[name=meeting_type]:checked").val() === "pmi" &&
      $("#setting_option_jbh").prop("checked")
    ) {
      if (!$("#setting_option_needpassword").prop("checked")) {
        $("#setting_option_needpassword")
          .prop("checked", true)
          .trigger("change");
      }
      if ($(".requirepwd > label .locked_by_admin").length > 0) {
      } else {
        $(".requirepwd > label")
          .append(o)
          .parent()
  ...
  function z() {
    ...
    if (e && "0" !== e["PMI"]) {
      ...
      $("[name=meeting_type][value=onetime]").prop({ checked: true });
    }
    ...
    if (
      ...
      $("[name=meeting_type][value=pmi]").prop({ checked: true });
    ...
    $("[name=option_video_host][value=" + s + "]").prop({ checked: true });
    ...
    if (e && !e["option_has3rd"]) {
      $(".audio>div.controls #option_audio_3rd")
        .parent()
        .remove();
      if ("other" == c) c = "both";
      $("#option_audio_both")
    ...
    if (e && e["option_noboth"]) {
      $(".audio>div.controls #option_audio_both")
        .parent()
        .remove();
      if ("both" == c) c = "telephony";
    }
    $("[name=option_audio][value=" + c + "]").prop({ checked: true });
    if (t && t.audio_locked) {
    ...
    if (e && e["option_enable_watermark"]) {
      ...
      $("#setting_option_watermark")
        .parents("div.control-row")
        .remove();
    }
    if (
      e &&
      e["scheduleDefaultLockOption"] &&
      "true" === e["scheduleDefaultLockOption"].audioWaterMark
    ) {
    ...
    if (e && e["recording"] && e["recording"].option_recording) {
      ...
      if (
        e &&
        e["recording"].option_local_recording &&
        e["recording"].option_cloud_recording
      ) {
        if (!$("#setting_recording_type").prop("checked")) {
          $(".recording-radio").hide();
        }
        if (2 == h) {
          $("[name=option_rec_type][value=" + h + "]").prop({ checked: true });
        } else {
          $("[name=option_rec_type][value=1]").prop({ checked: true });
        }
      } else if (e && e["recording"].option_local_recording) {
        $("#setting_recording_type")
          .next()
          .append(" on the local computer");
    ...
    if (e && e["option_enable_reg"] === true) {
      if (
        ...
        $("#setting_option_requirereg").prop({ disabled: true });
    ...
    if (e) {
      if (e["alt_hosts"] === undefined) {
        ...
        $("#alternative_host").val(
          COMMON.getValue("zoom_config_althost") || ""
        );
      }
    } else {
      $(".altnativehost").hide();
    ...
    $("#fixed_popup").prop(
      "checked",
      !!COMMON.getValue("zoom_config_fixed_popup")
    );
    ...
    if (a && a.option_pac_enable) {
      ...
      $(".pac")
        .find(".pac-mtg-number")
        .text(O.meeting_number);
      if (O.did_numbers && O.did_numbers != "") {
        $(".pac")
          .find(".pac-did-numbers")
          .html("<label>" + L(O.did_numbers) + "</label>");
      } else {
        $(".pac")
          .find(".pac-did-numbers")
          .parent()
    ...
    if (
      COMMON.getValue("zoom_config_fte") &&
      parseInt(COMMON.getValue("zoom_options_tabid")) > 0
    ) {
  ...
  function V() {
    ...
    $("[name=option_pac]").change(function() {
      ...
      if (a && a.option_pac_enable) {
        ...
        $(".pac")
          .find(".pac-mtg-number")
          .text(t.meeting_number);
        if (t.did_numbers && t.did_numbers != "") {
          $(".pac")
            .find(".pac-did-numbers")
            .html("<label>" + L(t.did_numbers) + "</label>");
          $(".pac")
            .find(".pac-did-numbers")
            .parent()
            .show();
    ...
    $("#setting_submit").click(function() {
      ...
      if (
        $("#setting_option_needpassword").prop("checked") &&
        t.is(":visible")
      ) {
        if ("" == t.val().trim()) {
      ...
      if (
        $(".topnav").is(":visible") &&
        $(".topnav li:last-child").hasClass("active")
      ) {
      ...
      COMMON.saveValue(
        ...
        $("#setting_option_uponentry").prop("checked")
      );
      COMMON.saveValue(
        "zoom_config_waiting_room",
        $("#setting_option_waitingroom").prop("checked")
      );
      COMMON.saveValue(
        "zoom_config_public_cal",
        $("#setting_option_public_cal").is(":visible") &&
          $("#setting_option_public_cal").prop("checked")
      );
      COMMON.saveValue(
        "zoom_config_video_host",
        $("[name=option_video_host]:checked").val()
      );
      COMMON.saveValue(
        "zoom_config_video_participants",
        $("[name=option_video_participants]:checked").val()
      );
      COMMON.saveValue(
        "zoom_config_audio",
        $("[name=option_audio]:checked").val()
      );
      COMMON.saveValue(
        "zoom_config_pacindex",
        $("[name=option_pac]:checked").val() || "1"
      );
      COMMON.saveValue(
        "zoom_config_watermark",
        $("#setting_option_watermark").is(":visible") &&
          $("#setting_option_watermark").prop("checked")
      );
      COMMON.saveValue(
        "zoom_config_audiomark",
        $("#setting_option_audiomark").is(":visible") &&
          $("#setting_option_audiomark").prop("checked")
      );
      COMMON.saveValue(
        "zoom_config_joinurl",
        $("#setting_option_joinurl").prop("checked")
      );
      COMMON.saveValue(
        "zoom_option_require_reg",
        $("#setting_option_requirereg").prop("checked")
      );
      ...
      if ($("#setting_recording_type").is(":visible")) {
        if ($("#setting_recording_type").prop("checked")) {
          if (
            a &&
            a["recording"].option_local_recording &&
            a["recording"].option_cloud_recording
          ) {
      ...
      COMMON.saveValue(
        "zoom_config_fixed_popup",
        $("#fixed_popup").is(":visible") && $("#fixed_popup").prop("checked")
      );
      let r = J();
      let s =
          $("#setting_option_authentication").is(":visible") &&
          $("#setting_option_authentication").prop("checked"),
        l;
      ...
      if (
        ...
        COMMON.saveZoomSetting(
          ...
          function() {
            ...
            chrome.tabs.update(
              parseInt(COMMON.getValue("zoom_options_tabid")),
  ...
  function D() {
    ...
    if (!(isNaN(e) || isNaN(t))) {
      chrome.tabs.update(t, { active: true }, function() {
  ...
  function P() {
    chrome.tabs.query({ active: true, currentWindow: true }, function(e) {
      let t = e[0].id;
      chrome.tabs.create(
        { url: chrome.extension.getURL("popup.html"), active: true },
        function(e) {
  ...
  function L(e) {
    return DOMPurify.sanitize(e, { SAFE_FOR_JQUERY: true });
  }
  function E(o, a) {
    let r = false;
    let s = false;
    ...
    $("[name=option_password_type][value=" + l + "]").prop({ checked: true });
    if (s) {
  ...
  function Q(e, t) {
    ...
    if (o.type == "1") {
      ...
      let a = `\n\t\t\t<span class="domain-info-text">${L(
        t
      )}</span><span class="edit-button">${n}</span>\n\t\t\t`;
      p.find(".domain-info-field")
        .html(a)
        .addClass("eyeable");
    } else {
      p.find(".domain-info-field")
